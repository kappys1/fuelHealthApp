...
/**
 * Cuerpo de una sesión en bloques numerados. Se extrae de la ficha (F26 Fase 2)
 * para que la **sesión adaptada del día** se lea EXACTAMENTE igual que la del
 * plan: mismos bloques, mismos rótulos, mismos grupos de F25.
 */
export function TrainingContentBlocks({ contenido }: { contenido: string }) {
  /*
  ...
  */
  const blocks = splitTrainingContent(contenido).map((block) => {
...
export function TrainingSessionDetail({
  ...
  actions?: ReactNode;
  className?: string;
  /** F26: «DEL PLAN» cuando el día tiene además una sesión adaptada (AC8). */
  badge?: string | null;
}) {
  return (
    <article className={cn("wellness-card overflow-hidden p-5", className)}>
      {badge ? (
        <p className="mb-2 inline-flex rounded-full bg-surface-2 px-2.5 py-1 text-[10px] font-bold tracking-wide text-muted-foreground ring-1 ring-line">
          {badge}
        </p>
      ) : null}
      ...
      <TrainingContentBlocks contenido={session.contenido} />
