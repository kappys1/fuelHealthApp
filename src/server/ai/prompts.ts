...
export function coachPrompt(args: {
  ...
  const header = `${args.atleta} Objetivos diarios: ${args.kcal} kcal, ${args.prot} g proteína, ~${args.carb} g hidratos, ~${args.fat} g grasa. En fases de Carga/Competición, superar kcal es esperado.`;
  // Guardarraíles (doc 10 A3): anti-suplementación (paridad con el chat) y
  // anti-entreno-fantasma. Solo suplementos de su perfil; si hoy es descanso o no
  // hay sesión, no asumir entreno ni dar timing pre/post.
  const guardrails = `Observas y explicas; NO prescribes suplementación. Si sugieres suplementos, SOLO los de su perfil; nada fuera de esa lista. Prioriza comida real y las comidas del plan que le quedan. Si la sesión de hoy es Descanso o no hay sesión, NO asumas que va a entrenar ni des timing pre/post-entreno.`;
  const block =
    args.mode === "hoy"
...
export function chatSystemPrompt(args: {
  ...
  const base = `${args.atleta} Respondes SOLO con base en los datos proporcionados. Si los datos no lo cubren, dilo. Observas y explicas; NO prescribes cambios de dieta ni suplementación — eso corresponde a su nutricionista (puedes sugerir qué preguntarle). Respuestas concisas, en español, con cifras concretas de sus datos.`;
  const sections = [
    base,
    `DIETA VIGENTE:\n${args.planSummary}`,
