...
src/
  app/
    (auth)/login/
    (app)/            # layout con nav inferior (4 pestañas, ver 09-FLUJOS-UX §2)
      hoy/            # registro diario (pantalla principal)
      plan/
      progreso/       # segmentos Tendencia | MED
      chat/
      ajustes/        # tema, import/export, estado sync Salud, mapeo sesión-día
    api/
...
- Sin analytics de terceros. Los datos de salud no salen de Vercel/Neon + el proveedor de IA configurado (solo lo mínimo por request).
